...
class AdminController < ApplicationController
  ...
  def index
    ...
    @quick_stats = {
      ...
      avg_sale: Order.where(created_at: Time.zone.now.midnight..Time.zone.now).average(:total)&.round(),
      per_sale: OrderProduct.joins(:order)
                            .where(orders: { created_at: Time.zone.now.midnight..Time.zone.now })&.average(:quantity),
    }
